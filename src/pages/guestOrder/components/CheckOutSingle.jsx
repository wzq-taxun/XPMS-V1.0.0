  ...
  useEffect(() => {
    if (props.visible) {
      ...
      if (props.isJoin) {
        ...
        getCloseSingleAccount(props.orderInfo.id).then(rsp => {
          if (rsp && rsp.code == Constants.SUCCESS) {
            const data = rsp.data || {};
            const { js = {}, ...order } = data;
            ...
            setList(list);
            const receive = parseFloat(data.ys || 0);
            setReceive(receive);
            const extra = parseFloat(js.all_charge || 0);
            setExtra(extra);
    ...
    props.form.validateFields((err, values) => {
      if (!err) {
        ...
        if (dispatch) {
          ...
          closeJoinWithExtra(order).then(rsp => {
            setLoading(false);
            dispatch({
              type: 'global/changeLoading',
              payload: false,
