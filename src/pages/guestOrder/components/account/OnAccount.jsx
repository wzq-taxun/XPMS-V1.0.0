...
import { getOnAccountType, getOnCompany } from '@/services/account';
...
const OnAccount = props => {
  ...
  useEffect(() => {
    ...
    if (hasDis) {
      message.error('包含不可挂账账务');
      return;
    }

    if (props.selectAccountCharge <= 0) {
      message.error('所选账务总金额为负,不可挂账,请走退款');
      return;
    }
    ...
    getOnCompany().then(rsp => {
      if (rsp && rsp.code == Constants.SUCCESS) {
        ...
        const select = data.filter(item => item.id == props.company_info_id);
        if (select && select.length > 0) {
          const {
            form: { setFieldsValue },
          } = props;
          setFieldsValue({ company_id: select[0].id });
        }
      }
    });
  ...
  const handleSubmit = e => {
    ...
    props.form.validateFields((err, values) => {
      if (!err) {
        ...
        if (!values.memo) {
          const companys = company.filter(item => (item.id == values.company_id));
          account.account.memo = companys && companys[0] && companys[0].name;
        }
...
export default connect(({ global }) => ({ loading: global.loading }))(Form.create()(OnAccount));
